...
export type WebinarFilterType = "all" | "online" | "offline"

const TABS: { type: WebinarFilterType; label: string }[] = [
  { type: "all", label: "전체" },
  { type: "online", label: "온라인 웨비나" },
  { type: "offline", label: "오프라인 세미나" },
]

export function WebinarFilterTabs({
  active,
  onChange,
}: {
  active: WebinarFilterType
  onChange: (type: WebinarFilterType) => void
}) {
  return (
